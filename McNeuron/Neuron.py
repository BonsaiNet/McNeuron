...
class Neuron(object):
    ...
    child_index : array of shape = [2, n_node]
        ...
        nodes are arrbitraty). second row: nan if neuron is order oner and
    ...
    def __init__(self, input_format = None, input_file = None):
        """
        Default constructor.
        Parameters
        -----------
        ...
        if input_format == 'Matrix of swc':
            ...
            # if(neuron_util.check_ordering(input_file[:, 6])):
            #     print("non-standard")
            #     input_file = neuron_util.make_standard_swc(input_file)
            # print(input_file)
    ...
    def set_features(self):
        ...
        self.features['Npassnode'] = \
            self.features['Nnodes'] - self.features['Nbranch']
        ...
        self.features['distance_from_parent'] = \
            self.distance_from_parent[self.n_soma:]
        self.features['distance_from_root'] = \
            self.distance_from_root[self.n_soma:]
        # self.features['x_location'] = \
        #     self.location[0, self.n_soma:]
        # self.features['y_location'] = \
        #     self.location[1, self.n_soma:]
        # self.features['z_location'] = \
        #     self.location[2, self.n_soma:]
        neural_all = \
            np.nansum(self.connection[self.n_soma:, self.n_soma:], axis=1)
        self.features['ratio_euclidian_neuronal'] = \
            neural_all/self.distance_from_root[self.n_soma:]
        self.features['ratio_diameter_euclidean'] = \
            self.set_diameter_euclidean()
        self.features['neuronal/euclidean'] = \
            np.array([((self.features['ratio_euclidian_neuronal'] - 1.)).mean()])
        x = np.abs(self.branch_angle[0, self.n_soma:])
        self.features['branch_angle'] = x[~np.isnan(x)]
        x1 = np.abs(self.branch_angle[1, self.n_soma:])
        x2 = np.abs(self.branch_angle[2, self.n_soma:])
        x = np.append(x1, x2)
        self.features['side_branch_angle'] = x[~np.isnan(x)]
        ...
        if(len(self.features['side_branch_angle']) == 0):
            self.features['side_branch_angle'] = np.array([0])
        value_dis, value_avoid = \
            self.set_discrepancy(np.arange(2, 120))
        self.features['discrepancy_space'] =value_dis
        self.features['self_avoidance'] =value_avoid
        self.features['pictural_image_xy'] = self.set_pictural(10., 10.)
        self.features['pictural_image_xy_tips'] = self.set_pictural_tips(10., 10.)
        self.set_curvature()
        important_node = self.get_index_main_nodes()
        parent_important = self.parent_index_for_node_subset(important_node)
        (neural, euclidan) = self.get_neural_and_euclid_lenght(important_node,
                                                               parent_important)
        self.features['neural_important'] = neural/self.distance_from_parent.mean()
        self.features['euclidian_important'] = euclidan
        self.features['ratio_neural_euclidian_important'] = neural/euclidan
        self.features['segmental_neuronal/euclidean'] = \
            np.array([((self.features['ratio_neural_euclidian_important'] - 1.)).mean()])
        self.features['branch_angle_segment'] = \
            self.set_branch_angle_segment(important_node, parent_important)
    ...
    def set_pictural(self, x_mesh, y_mesh):
        """
    ...
    def set_branch_order(self):
        ...
        self.branch_order = np.zeros(self.n_node)
        unique, counts = np.unique(self.parent_index[1:], return_counts=True)
        self.branch_order[unique] = counts
    ...
    def set_pictural_tips(self, x_mesh, y_mesh):
        image = np.zeros(int(x_mesh * y_mesh))
        (I,) = np.where(self.branch_order==0)
    ...
    def set_curvature(self):
        ...
        self.features['curvature'] = I[self.n_soma:]

    def set_diameter_euclidean(self, bins=10):
        dis = self.features['distance_from_root']
        dis = dis/max(dis)
        M = np.zeros(bins)
        for i in range(bins):
            (index_less,) = np.where(dis < (i+1)*.1)
            (index_higher,) = np.where(i*.1 < dis)
            index = np.intersect1d(index_less, index_higher)
            dia = self.diameter[self.n_soma + index]
            if(len(dia)==0):
                m = 0
            else:
                m = dia.mean()
    ...
    def rotate(self, node, matrix):
        ...
        self.set_distance_from_root()
        self.set_distance_from_parent()
        self.adjust_connection_with_parent()
        self.set_branch_angle()
        self.set_global_angle()
        self.set_local_angle()
        self.set_features()
    ...
    def connecting_after_node(self, node):
        """
    ...
    def read_swc_matrix(self, swc_matrix):
        ...
        # n_node = swc_matrix.shape[0]
        # for line in range(n_node):
        #     index = swc_matrix[line,0]
        #     swc_type = swc_matrix[line,1]
        #     x = swc_matrix[line,2]
        #     y = swc_matrix[line,3]
        #     z = swc_matrix[line,4]
        #     radius = swc_matrix[line,5]
        #     parent_index = int(swc_matrix[line,6])
        #     if(parent_index == -1):
        #         self.n_soma += 1
        #         x_root = x
        #         y_root = y
        ...
        #         self.nodes_type = np.append(self.nodes_type, swc_type)
