...
import warnings
import McNeuron.subsample as subsample
...
class Neuron:
    ...
    def l_measure_features(self):
        ...
        self.location[0,:] = self.location[0,:] - self.location[0,0]
        self.location[1,:] = self.location[1,:] - self.location[1,0]
        self.location[2,:] = self.location[2,:] - self.location[2,0]
        
        self.features['Width X'] = max(self.location[0,:]) - min(self.location[0,:])
        self.features['Height Y'] = max(self.location[1,:]) - min(self.location[1,:])
        ...
        self.features['Skewness X'] = np.abs(self.location[0,:].mean())
        self.features['Skewness Y'] = np.abs(self.location[1,:].mean())
        self.features['Skewness Z'] = np.abs(self.location[2,:].mean())
        ...
        self.features['Euclidian Skewness'] = np.sqrt(np.square(self.location[0,:].mean()) + np.square(self.location[1,:].mean()) + np.square(self.location[2,:].mean()))
    ...
    def setNormalizedFeatures(self) :
        ...
        standard_swc[:,2] = standard_swc[:,2] - standard_swc[0,2]
        standard_swc[:,3] = standard_swc[:,3] - standard_swc[0,3]
        standard_swc[:,4] = standard_swc[:,4] - standard_swc[0,4]
        location = standard_swc[:,2:5]
        pca = PCA(n_components=3)
        pca.fit(location)
        embedded = pca.fit_transform(location)
        
        soma_indices = np.where(self.nodes_type==1)[0]
        self.basic_features()
        
        self.features['Width X'] = embedded[:,0].max() - embedded[:,0].min()
        self.features['Height Y'] = embedded[:,1].max() - embedded[:,1].min()
        self.features['Depth Z'] = embedded[:,2].max() - embedded[:,2].min()
        self.features['Soma Radii'] = (self.diameter[soma_indices].mean())/(self.features['Width X'])
        self.features['Soma Surface Area'] = (self.somaSurfaceArea(soma_indices))/(self.features['Width X'] * self.features['Height Y'])
        self.features['Soma Volume'] = (self.somaVolume(soma_indices))/(self.features['Width X'] * self.features['Height Y'] * self.features['Depth Z'])
        self.features['Skewness X'] = (np.abs(self.location[0,:].mean()))/(self.features['Width X'])
        self.features['Skewness Y'] = (np.abs(self.location[1,:].mean()))/(self.features['Height Y'])
        self.features['Skewness Z'] = (np.abs(self.location[2,:].mean()))/(self.features['Depth Z'])
        self.features['Euclidian Skewness'] = (np.sqrt(np.square(self.location[0,:].mean()) + np.square(self.location[1,:].mean()) + np.square(self.location[2,:].mean())))/(self.features['Width X'] * self.features['Height Y'] * self.features['Depth Z'])
        self.features['Length'] = (self.totalLength())/(self.features['Width X'])
        self.features['Surface Area'] = (self.surfaceArea())/(self.features['Width X'] * self.features['Height Y'])
        self.features['Section Area'] = (np.sum(np.pi*(self.diameter[1:]**2)))
        self.features['Volume'] = (self.volume())/(self.features['Width X'] * self.features['Height Y'] * self.features['Depth Z'])
        self.features['Average Radius'] = (self.diameter.mean())/(self.features['Width X'])
